...
# torch.distributed.init_process_group(backend='nccl', init_method=None, timeout=datetime.timedelta(seconds=1800), store=None, group_name='', pg_options=None)

deepspeed.init_distributed(dist_backend='nccl', init_method=None, timeout=datetime.timedelta(seconds=1800))
...
def load_tokenizer(pretrained_model_name_or_path: str = DEFAULT_INPUT_MODEL) -> PreTrainedTokenizer:
    ...
    tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path, return_token_type_ids=False)
...
#     config = AutoConfig.from_pretrained("./polyglot-ko-5.8b")

    model = AutoModelForCausalLM.from_pretrained(
        pretrained_model_name_or_path, trust_remote_code=True, use_cache=False if gradient_checkpointing else True,device_map="cuda"
...
def train(
    *,
    ...
    device = accelerator.device  #################################################accelerator
    dp.init_distributed(dist_backend='nccl', init_method=None, timeout=datetime.timedelta(seconds=1800))
...
#     trainer= accelerator.prepare(trainer) #################################################accelerator
    
    logger.info("Training")
    model.config.use_cache = False
    notebook_launcher(trainer.train(), num_processes=6, mixed_precision="bf16")
#     trainer.train()
...
# notebook_launcher(trainer.train(), num_processes=4, mixed_precision="bf16")
...
def main(**kwargs):
    train(**kwargs)
#     notebook_launcher(train(**kwargs), num_processes=4, mixed_precision="bf16")
#     notebook_launcher(train(**kwargs), num_processes=4)
